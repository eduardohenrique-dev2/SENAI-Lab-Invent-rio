...
-- Execute no MESMO projeto Supabase usado pelo ecossistema SENAI Lab.
-- ============================================================
...
-- Reaproveita os perfis do Sistema de Gestão de Demandas quando disponíveis.
do $$
begin
...
as $$
    ...
    where p.user_id=(select auth.uid()) and p.ativo=true
    limit 1;
$$;
...
create unique index if not exists inv_categorias_nome_tipo_idx on public.inv_categorias (lower(nome),tipo) where ativo=true;
create unique index if not exists inv_localizacoes_codigo_idx on public.inv_localizacoes (lower(codigo)) where codigo is not null and ativo=true;

-- ============================================================
-- 3. MOVIMENTAÇÕES, EMPRÉSTIMOS E MANUTENÇÕES
-- ============================================================
create table if not exists public.inv_movimentacoes (
    ...
    tipo text not null check (tipo in ('entrada','saida','transferencia','emprestimo','devolucao','manutencao','baixa','ajuste')),
    quantidade numeric(14,3) not null default 1 check (quantidade > 0),
...
-- ============================================================
-- 5. NOTIFICAÇÕES, CONFIGURAÇÕES E AUDITORIA
-- ============================================================
create table if not exists public.inv_notificacoes (
    id bigint generated always as identity primary key,
    user_id uuid references auth.users(id) on delete cascade,
    severidade text not null default 'info' check (severidade in ('info','warning','danger','success')),
    titulo text not null,
...
declare
    ...
    v_dados jsonb;
begin
    v_id := (case when tg_op='DELETE' then to_jsonb(old) else to_jsonb(new) end) ->> 'id';
...
declare t text;
begin
    foreach t in array array['inv_perfis','inv_localizacoes','inv_itens','inv_emprestimos','inv_manutencoes'] loop
...
declare t text;
begin
    foreach t in array array['inv_itens','inv_localizacoes','inv_movimentacoes','inv_emprestimos','inv_manutencoes','inv_inventarios_fisicos','inv_contagens','inv_perfis'] loop
...
revoke all on public.inv_perfis,public.inv_categorias,public.inv_localizacoes,public.inv_itens,
    public.inv_movimentacoes,public.inv_emprestimos,public.inv_manutencoes,
    public.inv_inventarios_fisicos,public.inv_contagens,public.inv_notificacoes,
    public.inv_configuracoes,public.inv_auditoria from anon, authenticated;

grant select on public.inv_perfis,public.inv_categorias,public.inv_localizacoes,public.inv_itens,
    public.inv_movimentacoes,public.inv_emprestimos,public.inv_manutencoes,
    public.inv_inventarios_fisicos,public.inv_contagens,public.inv_notificacoes,
    public.inv_configuracoes,public.inv_auditoria to authenticated;

grant insert,update,delete on public.inv_categorias,public.inv_localizacoes to authenticated;
grant insert,update on public.inv_itens,public.inv_manutencoes,public.inv_inventarios_fisicos,public.inv_contagens,public.inv_configuracoes to authenticated;
grant insert on public.inv_movimentacoes to authenticated;
grant update on public.inv_emprestimos,public.inv_notificacoes,public.inv_perfis to authenticated;

-- remove políticas desta migration para permitir reexecução segura
do $$
declare r record;
begin
    for r in
        ...
        from pg_policies
        where schemaname='public' and policyname like 'inv %'
    loop
...
using (user_id=(select auth.uid()) or private.inv_papel() in ('administrador','gestor','auditor'));
...
create policy "inv itens leitura interna" on public.inv_itens
for select to authenticated
using (private.inv_ativo() and private.inv_papel() <> 'aluno');
...
for select to authenticated using (private.inv_pode_operar() or private.inv_pode_auditar());
...
for select to authenticated
using (private.inv_pode_operar() or private.inv_pode_auditar() or solicitado_por=(select auth.uid()));
...
for select to authenticated using (private.inv_pode_gerir() or user_id=(select auth.uid()));
...
returns table (
    id uuid,codigo_interno text,patrimonio text,nome text,tipo_item text,
    marca text,modelo text,status text,localizacao text,foto_url text,descricao text
)
language sql
stable
security definer
set search_path = ''
as $$
    select i.id,i.codigo_interno,i.patrimonio,i.nome,i.tipo_item,
           i.marca,i.modelo,i.status,l.nome,i.foto_url,i.descricao
    from public.inv_itens i
    left join public.inv_localizacoes l on l.id=i.localizacao_id
    where i.qr_token=p_token and i.ativo=true
    limit 1;
$$;
...
create or replace function public.inv_listar_itens_consulta()
returns table (
    id uuid,codigo_interno text,patrimonio text,nome text,tipo_item text,
    marca text,modelo text,status text,localizacao_id uuid,localizacao text,
    foto_url text,descricao text,quantidade numeric,unidade text,qr_token uuid
)
language plpgsql
stable
security definer
...
begin
    ...
    return query
    select i.id,i.codigo_interno,i.patrimonio,i.nome,i.tipo_item,
           i.marca,i.modelo,i.status,i.localizacao_id,l.nome,
           i.foto_url,i.descricao,i.quantidade,i.unidade,i.qr_token
    from public.inv_itens i
    left join public.inv_localizacoes l on l.id=i.localizacao_id
    where i.ativo=true
    order by i.nome;
...
revoke execute on function public.inv_listar_itens_consulta() from public, anon;
grant execute on function public.inv_listar_itens_consulta() to authenticated;
...
begin
    if not private.inv_pode_operar() then raise exception 'Sem permissão para movimentar itens.'; end if;
    if p_tipo not in ('entrada','saida','transferencia','emprestimo','devolucao','manutencao','baixa','ajuste') then raise exception 'Tipo inválido.'; end if;

    select * into v_item from public.inv_itens where id=p_item_id and ativo=true for update;
    if not found then raise exception 'Item não encontrado.'; end if;

    if p_tipo<>'transferencia' and v_qtd<=0 then raise exception 'Quantidade inválida.'; end if;
    ...
    if p_tipo='entrada' then
        ...
        if p_destino is null then raise exception 'Informe o destino.'; end if;
...
create or replace function public.inv_solicitar_emprestimo(
    p_item_id uuid,p_quantidade numeric,p_nome text,p_email text,
    p_previsao timestamptz,p_estado text default '',p_observacoes text default ''
)
returns uuid
language plpgsql
security definer
...
begin
    ...
    v_status:=case when private.inv_pode_operar() then 'aberto' else 'solicitado' end;

    insert into public.inv_emprestimos (
        id,item_id,quantidade,retirado_por_nome,retirado_por_email,data_retirada,
        previsao_devolucao,autorizado_por,solicitado_por,estado_retirada,
        observacoes,assinatura_confirmada,status
    ) values (
        ...
        case when v_status='aberto' then (select auth.uid()) else null end,
        (select auth.uid()),trim(coalesce(p_estado,'')),trim(coalesce(p_observacoes,'')),
        v_status='aberto',v_status
    );
...
begin
    ...
    if not found or i.quantidade<v.quantidade then raise exception 'Quantidade indisponível.'; end if;
...
create or replace function public.inv_devolver_emprestimo(
    p_emprestimo_id uuid,p_estado text default '',p_observacoes text default ''
)
returns boolean
language plpgsql
security definer
set search_path = ''
as $$
declare v public.inv_emprestimos%rowtype;
begin
    if not private.inv_pode_operar() then raise exception 'Sem permissão.'; end if;
    select * into v from public.inv_emprestimos where id=p_emprestimo_id for update;
...
create or replace function public.inv_criar_inventario_fisico(
    p_titulo text,p_responsavel text,p_observacoes text default ''
)
returns uuid
language plpgsql
security definer
...
values ('inventario-publico','inventario-publico',true,20971520,array['image/png','image/jpeg','image/webp','application/pdf'])
...
for select to public using (bucket_id='inventario-publico');

drop policy if exists "inventario arquivos inserir" on storage.objects;
create policy "inventario arquivos inserir" on storage.objects
for insert to authenticated
with check (bucket_id in ('inventario-publico','inventario-privado') and private.inv_ativo());

drop policy if exists "inventario privados ler" on storage.objects;
create policy "inventario privados ler" on storage.objects
for select to authenticated
...
create policy "inventario arquivos atualizar" on storage.objects
for update to authenticated
...
create policy "inventario arquivos apagar" on storage.objects
...
begin
    if exists(select 1 from pg_publication where pubname='supabase_realtime') then
        foreach t in array array['inv_itens','inv_movimentacoes','inv_emprestimos','inv_manutencoes','inv_notificacoes'] loop
            if not exists(
